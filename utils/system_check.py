...
def check_system_utilities() -> bool:
    ...
    """
    required_utils = ["curl", "git", "python3"]
    required_libs = ["/usr/lib/x86_64-linux-gnu/libGL.so.1"]
    ...
    for util in required_utils:
        try:
            ...
            if result.returncode != 0:
                logger.error(f"Утилита {util} не найдена")
                return False
        except Exception as e:
    ...
    for lib in required_libs:
        if not os.path.exists(lib):
            logger.error(f"Библиотека {lib} не найдена")
            return False
