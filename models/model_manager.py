...
class ModelManager(QObject):
    ...
    def execute_system_command(self, model_id: str, command: str, args: List[str]) -> Dict[str, any]:
        """
        Выполнение системной команды через ИИ
        ...
        try:
            ...
            if not self.is_model_installed(model_id):
                raise Exception(f"Модель {model_id} не установлена")
                
            # Проверка команды через SystemManager
            result = SystemManager.execute_command(command, args)
            
            logger.info(f"Выполнена команда {command} от модели {model_id}")
            return result
            
        except Exception as e:
            logger.error(f"Ошибка выполнения команды от модели {model_id}: {str(e)}")
            raise e
            
    def modify_program(self, model_id: str, file_path: str, new_content: str) -> bool:
        """
        Модификация файла программы через ИИ
        ...
        try:
            ...
            logger.info(f"Модифицирован файл {file_path} моделью {model_id}")
            return True
            
        except Exception as e:
            logger.error(f"Ошибка модификации файла моделью {model_id}: {str(e)}")
            return False
